...
When a parameter of a WebIDL method is declared as `Promise<T>`, it will automatically resolve any arguments passed in that position, thus taking care of the "Promise Arguments Should Be Resolved" advice above.
...
As a consequence of the resolution behavior, `Promise<T>` parameters cannot be overloaded with any other parameters.
...
TODO make these better, more real-world
...
interface ThirdThing {
  readonly attribute Promise<DOMString> g;
}
```

```webidl
// Promise-accepting methods
interface FourthThing {
